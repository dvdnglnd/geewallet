...
module FSharpUtil =
        ...
        let ToStringFormat (fmt: string) =
            let rec inner (innerFmt: string) (count: uint32) =
                ...
                if formatsFound.Any () then
                    ...
                    let x =
                        sub.Replace (firstFormat, "{" + count.ToString () + "}")
                        + innerFmt.Substring (subEnd)
    ...
    module UwpHacks =
        ...
        let SPrintF2 fmt a b =
            sprintf fmt a b

        let SPrintF3 fmt a b c =
            sprintf fmt a b c

        let SPrintF4 fmt a b c d =
            sprintf fmt a b c d

        let SPrintF5 fmt a b c d e =
            sprintf fmt a b c d e
#else
        let SPrintF1 (fmt: string) (a: Object) =
