...
module Server =
    ...
    let HttpRequestExceptionMatchesErrorCode (ex: Http.HttpRequestException) (errorCode: int): bool =
        ex.Message.StartsWith (SPrintF1 "%i " errorCode)
        || ex.Message.Contains (SPrintF1 " %i " errorCode)

    let exMsg = "Could not communicate with EtherServer"

    let PerformEtherRemoteCallWithTimeout<'T, 'R> (job: Async<'R>): Async<'R> =
        async {
    ...
    let MaybeRethrowHttpRequestException (ex: Exception): unit =
        ...
        | Some httpReqEx ->
            ...
            if HttpRequestExceptionMatchesErrorCode httpReqEx (int CloudFlareError.OriginSslHandshakeError) then
                raise
                <| ServerChannelNegotiationException (exMsg, CloudFlareError.OriginSslHandshakeError, httpReqEx)
    ...
    let MaybeRethrowInnerRpcException (ex: Exception): unit =
        ...
        | Some rpcUnknownEx ->
            ...
            | None -> ()
            | Some deserEx -> raise <| ServerMisconfiguredException (deserEx.Message, ex)
            ...
            | Some httpReqEx ->
                if httpReqEx.Message.Contains "SSL" then
                    ...
                    | None ->
                        ...
                        | Some secEx -> raise <| ProtocolGlitchException (secEx.Message, ex)
                        | None -> ()
            | None -> ()
        | None -> ()
    ...
    let Web3ServerToRetrievalFunc (server: ServerDetails) (web3ClientFunc: SomeWeb3 -> Async<'R>): Async<'R> =
    ...
    let GetTransactionCount (currency: Currency) (address: string): Async<HexBigInteger> =
        async {
            let web3Funcs =
    ...
    let private GetBlockToCheckForConfirmedBalance (web3: Web3): Async<BlockParameter> =
        async {
            ...
            let blockToCheck =
                BigInteger.Subtract (latestBlock.Value, NUMBER_OF_CONFIRMATIONS_TO_CONSIDER_BALANCE_CONFIRMED)
...
*)
            ...
            let! balance =
                web3.Eth.GetBalance.SendRequestAsync (publicAddress, blockForConfirmationReference, null, cancelToken)
                |> Async.AwaitTask
            ...
            let web3Funcs =
                let web3Func (web3: Web3): Async<decimal> =
                    async {
                        let! balance =
                            match balType with
                            | BalanceType.Confirmed -> GetConfirmedEtherBalanceInternal web3 address
        ...
        async {
            ...
            let! balance =
                contractHandler.QueryAsync<BalanceOfFunction, BigInteger>
                    (balanceOfFunctionMsg, blockForConfirmationReference, cancelToken)
                |> Async.AwaitTask
        ...
        async {
            let web3Funcs =
                let web3Func (web3: Web3): Async<HexBigInteger> =
                    ...
                    let transferFunctionMsg =
                        TransferFunction (FromAddress = account.PublicAddress, To = destination, Value = amountInWei)
    ...
    let GetGasPrice (currency: Currency): Async<HexBigInteger> =
        async {
            let web3Funcs =
                let web3Func (web3: Web3): Async<HexBigInteger> =
                    async {
    ...
    let BroadcastTransaction (currency: Currency) (transaction: string): Async<string> =
        ...
        async {
            ...
            try
                return! faultTolerantEtherClient.Query (FaultTolerantParallelClientSettingsForBroadcast ()) web3Funcs
            with ex ->
                ...
                | Some rpcResponseException ->
                    ...
                    if rpcResponseException.Message.StartsWith
                        (insufficientFundsMsg, StringComparison.InvariantCultureIgnoreCase) then
