...
type internal Runner<'Resource when 'Resource: equality> =
    static member Run<'K, 'Ex when 'K: equality and 'K :> ICommunicationHistory and 'Ex :> Exception> (server: Server<'K, 'Resource>)
                                                                                                      (stopwatch: Stopwatch)
        ...
        async {
            ...
            with ex ->
                ...
                let isLateEnoughToReportProblem (state: ClientCancelStateInner) =
                    match state with
                    | Alive _ -> false
                    | Canceled date -> (date + TimeSpan.FromSeconds 1.) < DateTime.UtcNow
                ...
                | Some specificInnerEx ->
                    if report then
                        Infrastructure.LogError (SPrintF1 "Cancellation fault warning: %s" (ex.ToString ()))
    ...
    static member CreateAsyncJobFromFunc<'K, 'Ex when 'K: equality and 'K :> ICommunicationHistory and 'Ex :> Exception> (shouldReportUncanceledJobs: bool)
                                                                                                                         (exceptionHandler: Option<Exception -> unit>)
                                                                                                                         (cancelState: ClientCancelState)
    ...
    static member CreateJobs<'K, 'Ex when 'K: equality and 'K :> ICommunicationHistory and 'Ex :> Exception> (shouldReportUncanceledJobs: bool)
                                                                                                             (parallelJobs: uint32)
                                                                                                             (exceptionHandler: Option<Exception -> unit>)
        ...
        let launchFunc =
            Runner.CreateAsyncJobFromFunc<'K, 'Ex>
                shouldReportUncanceledJobs
                exceptionHandler
                cancelState
                updateServerFunc
...
type FaultTolerantParallelClient<'K, 'E when 'K: equality and 'K :> ICommunicationHistory and 'E :> Exception> (updateServer: ('K -> bool) -> HistoryFact -> unit) =
    do
    ...
    let MeasureConsistency (results: List<'R>) =
        results
        |> Seq.countBy id
        |> Seq.sortByDescending (fun (_, count: int) -> count)
        |> List.ofSeq

    let LaunchAsyncJob (job: ServerJob<'K, 'R>): ServerTask<'K, 'R> =
        let cancellationSource = new CancellationTokenSource()

        let token =
            try
                cancellationSource.Token
            with :? ObjectDisposedException as ex ->
                raise
                <| TaskUnavailabilityException ("cancellationTokenSource already disposed", ex)
    ...
    let rec WhenSomeInternal (consistencySettings: Option<ConsistencySettings<'R>>)
        ...
        async {
            if startedTasks = List.Empty then
                ...
                let restOfTasks = startedTasks.Where (fun task -> not (task = fastestTask)) |> List.ofSeq

                let newResults, newFailedFuncs =
                    match fastestTask.Task.Result with
                    | Failure unsuccessfulServer -> resultsSoFar, unsuccessfulServer :: failedFuncsSoFar
                    | SuccessfulResult newResult -> newResult :: resultsSoFar, failedFuncsSoFar
    ...
    let WhenSome (settings: FaultTolerantParallelClientSettings<'R>)
        ...
        let shouldReportUncanceledJobs =
            ...
            | Exhaustive -> false
            | Selective subSettings -> subSettings.ReportUncanceledJobs
        ...
        let startedTasks, jobsToLaunchLater =
            ...
            | None -> raise <| TaskCanceledException "Found canceled when about to launch more jobs"
            | Some (firstJobsToLaunch, jobsToLaunchLater) ->
                match cancellationSource with
                | None -> ()
                | Some customCancelSource ->
                    try
                        customCancelSource.Canceled.Add (fun _ -> CancelAndDispose cancelState)
    ...
    let rec QueryInternalImplementation (settings: FaultTolerantParallelClientSettings<'R>)
        ...
        async {
            ...
            let job =
                WhenSome settings consistencyConfig funcs resultsSoFar failedFuncsSoFar cancellationSource

            let! result = job

            match result with
            | AverageResult averageResult -> return averageResult
            | ConsistentResult consistentResult -> return consistentResult
    ...
    member private __.QueryInternal<'R when 'R: equality> (settings: FaultTolerantParallelClientSettings<'R>)
        ...
        let maybeSortedServers =
            match settings.ResultSelectionMode with
            | Exhaustive -> servers
            | Selective selSettings -> SortServers servers selSettings.ServerSelectionMode
    ...
    member self.QueryWithCancellation<'R when 'R: equality> (cancellationTokenSource: CustomCancelSource)
